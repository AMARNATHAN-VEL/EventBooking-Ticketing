...
const AdminLayout = () => {
  ...
  return (
    <div
      className={`min-h-screen ${isDark ? "bg-[#05080f] text-white" : "bg-slate-100 text-slate-900"}`}
    >
      <aside
        className={`fixed left-0 top-0 z-20 h-screen w-72 border-r px-6 py-8 flex flex-col ${isDark ? "bg-[#081025] border-slate-800" : "bg-white border-slate-200"}`}
      >
        <div className="mb-10">
          <p
            className={`text-xs font-semibold uppercase tracking-[0.3em] ${isDark ? "text-white" : "text-cyan-700"}`}
          >
            Admin Portal
          </p>
        ...
        <div className="mt-auto rounded-3xl bg-surface p-4 text-sm text-on-light shadow-sm ring-1 ring-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p
                className={`text-xs ${isDark ? "text-white" : "text-slate-500"}`}
              >
                {adminProfile.role}
              </p>
            </div>
      ...
      {showProfile ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div
            className={`w-full max-w-md rounded-3xl p-6 shadow-2xl ring-1 ${isDark ? "bg-[#081025] border-slate-800" : "bg-white border-slate-200"}`}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <p
                  className={`text-sm font-semibold uppercase tracking-[0.2em] ${isDark ? "text-white" : "text-slate-500"}`}
                >
                  Admin profile
                </p>
                <h2
                ...
                <p
                  className={`mt-1 text-sm ${isDark ? "text-white" : "text-slate-500"}`}
                >
                  {adminProfile.role}
                </p>
              </div>
            ...
            <div className="mt-6 space-y-4 text-sm">
              <div>
                ...
                <p className={`${isDark ? "text-white" : "text-slate-600"}`}>
                  {adminProfile.email}
                </p>
              </div>
              <div>
